    ...
    try:
        ...
        if "contact information" in question.lower():
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an assistant helping to identify top lawyers. Return recommendations in a table with lawyer name, work email and work phone, and relevant cases. You can find the work email and work phone in the Users.CSV. Run through this file for this information.  Find the relevant cases from matter.csv and then also the lawyer information through users.csv before you make a recommendation. Often times the practice group may not actually reflect the actual law case so parse through all the csv files before recommending a lawyer. Make sure you go through all the information each csv and return out relevant cases from the matters csv but make sure you output the information for lawyer contact and name from users.csv. Return the lawyers work email and work phone if available."},
